...
import nn_fac.update_rules.nnls_tl as nnls
import nn_fac.update_rules.mu_tl as mu
...
def one_nmf_step(data, rank, U_in, V_in, norm_data, update_rule, beta,
    ...
    if 0 not in fixed_modes:
        ...
        if update_rule == "hals":
            ...
            VMt = tl.dot(V,tl.transpose(data))
            ...
            if deterministic:
                U = tl.transpose(nnls.hals_nnls_acc(VMt, VVt, tl.transpose(U_in), maxiter=100, atime=timer, alpha=math.inf, delta=0.01,
                                                sparsity_coefficient = sparsity_coefficients[0], normalize = normalize[0], nonzero = False)[0])
            else:
                U = tl.transpose(nnls.hals_nnls_acc(VMt, VVt, tl.transpose(U_in), maxiter=100, atime=timer, alpha=0.5, delta=0.01,
                                                sparsity_coefficient = sparsity_coefficients[0], normalize = normalize[0], nonzero = False)[0])
