...
def ntd(tensor, ranks, init = "random", core_0 = None, factors_0 = [], n_iter_max=100, tol=1e-6,
    ...
    More precisely, the chosen optimization algorithm may be the:
        -  HALS [2] for the factors, which updates each factor columnwise, fixing every other columns, 
        and a projected gradient for the core,which reduces the memory neccesited to perfome HALS on the core.
        The projected gradient rule is derived by the authors, as presented in [3].
        HALS optimizes the squared Euclidean distance between T and its approximation.
        -  Multiplicative Update (MU) rules [4] for both the factors and the core.
        When using MU, the algorithm optimizes the $\beta$-divergence loss [5].
        ...
        The Multiplicative Update rules are defined for NMF in [5] and redeveloped in [4].
        This algorithm is described in more details in [6]. 

    Tensors are manipulated with the tensorly toolbox [7].

    In tensorly and in our convention, tensors are unfolded and treated as described in [8].
    ...
    init: "random" | "tucker" | "chromas" | "custom"
        - If set to random:
    ...
    tol: float
        ...
        Default: 1e-6
    update_rule: "hals" | "mu"
        The update rule used for the optimization.
        "hals" optimizes the Euclidean norm only.
        "mu" optimizes the beta-divergence, for any beta parameter.
        Default: "hals"
    beta: float
        ...
        Other values are possible, see [5] for more details.$
        Default: 2
    sparsity_coefficients: list of float (as much as the number of modes + 1 for the core)
        ...
        Default: []
    fixed_modes: list of booleans (as much as the number of modes + 1 for the core)
        Indicates whether the factor (or the core, last entry) is fixed (not updated) or not.
        Default: []
    ...
    seed: integer
        The seed for the random initialization, if deterministic is set to True.
        Default: 0
    ...
    [3] Marmoret A., Cohen J.E., Bertin, N., Bimbot F. "Uncovering audio 
    patterns in music with nonnegative Tucker decomposition for structural 
    segmentation" In ISMIR 2020-21st International Society for Music 
    Information Retrieval, Nov 2020 
    ...
    [6] Marmoret, A., Voorwinden, F., Leplat, V., Cohen, J.E., and Bimbot, F.
    "Nonnegative tucker decomposition with beta-divergence for music structure 
    analysis of audio signals". In XXVIIIème Colloque Francophone de Traitement
    du Signal et des Images (GRETSI 2022), Nov 2022 

    [7] J. Kossai et al. "TensorLy: Tensor Learning in Python",
    arxiv preprint (2018)

    [8] Jeremy E Cohen. "About notations in multiway array processing",
    ...
    if type(ranks) is int: # if only one rank is provided, use it for all modes
        ranks = [ranks for i in range(nb_modes)]
    ...
    for i in range(nb_modes):
        if ranks[i] > tensor.shape[i]:
            ...
            warnings.warn(f"The {i}-th mode rank was larger than the shape of the tensor, which is incorrect (rank: {ranks[i]}, tensor shape: {tensor.shape[i]}). The rank was then set to the shape of the tensor.")

    if update_rule == "hals":
        assert beta == 2, f"Beta parameter is only used for MU update rule. Please set update_rule to 'mu' to use another beta value than 2. (Current setting: beta = {beta} and update_rule = {update_rule})."
    ...
    if init.lower() == "custom":
        ...
        core, factors = init_factors.ntd_initialization(tensor, ranks, init, deterministic=deterministic, seed=seed)
    
    if fixed_modes == None or len(fixed_modes) != nb_modes + 1:
        fixed_modes = [False for i in range(nb_modes + 1)]
    if init.lower() == "chromas":
        fixed_modes[0] = True
    ...
    return compute_ntd(tensor, ranks, core, factors, n_iter_max=n_iter_max, tol=tol,
                       ...
                       verbose=verbose, return_costs=return_costs, deterministic = deterministic, seed=seed)
...
def compute_ntd(tensor_in, ranks, core_in, factors_in, n_iter_max=100, tol=1e-6,
    ...
    or multiplicative updates (MU) [3], with factors_in 
    as initialization.
    ...
    beta: float
        ...
        1 - Kullback-Leibler divergence
        0 - Itakura-Saito divergence
        Other values are possible, see [3] for more details.
        Default: 2
    sparsity_coefficients: list of float (as much as the number of modes + 1 for the core)
        The sparsity coefficients on each factor and on the core respectively.
        If set to None or [], the algorithm is computed without sparsity
    ...
    if fixed_modes == None or len(fixed_modes) != nb_modes + 1:
        print("Irrelevant number of fixed modes booleans (different from the number of modes + 1 for the core), they have been set to False.")
        fixed_modes = [False for i in range(nb_modes + 1)]
    ...
    if update_rule == "hals" and beta != 2:
        raise err.InvalidArgumentValue(f"The hals is only valid for the frobenius norm, corresponding to the beta divergence with beta = 2. Here, beta was set to {beta}. To compute NTD with this value of beta, please use the mu update_rule.") from None
    ...
    for iteration in range(n_iter_max):
        ...
        match update_rule:
            ...
            case "mu":
                core, factors, cost = one_ntd_step_mu(tensor, ranks, core, factors, beta, norm_tensor, fixed_modes, normalize, mode_core_norm)

            case _:
                raise err.InvalidArgumentValue(f"The update rule provided is not valid. Please choose between 'hals' and 'mu' (Got {update_rule}).") from None
...
def one_ntd_step(tensor, ranks, in_core, in_factors, norm_tensor,
    ...
    modes_list = [mode for mode in range(tl.ndim(tensor)) if not fixed_modes[mode]]
    ...
    for mode in modes_list:
        ...
        factors[mode] = tl.transpose(nnls.compute_hals_nnls(V=tl.transpose(factors[mode]), UtM=UtM, UtU=UtU, 
    ...
    if not fixed_modes[-1]:
        while cnt <= 300 and upd>= delta * upd_0:
            ...
            gradient_update = gradient_step*gradient
            core = tl.clip(core - gradient_update, a_min=0)
            upd = tl.norm(gradient_update)
            if cnt == 1:
                upd_0 = upd
    ...
    for index, sparse in enumerate(sparsity_coefficients):
        if sparse:
            if index < len(factors):
                ...
                sparsity_error += 2 * (sparse * tl.norm(core, order=1))
            else:
...
def ntd_mu(tensor, ranks, init = "random", core_0 = None, factors_0 = [], n_iter_max=1000, tol=1e-6,
    ...
    """
    DEPRECATED
    Now move in the general ntd function.
    """
    raise DeprecationWarning("The ntd_mu function is deprecated. Please use the ntd function with update_rule='mu' instead.")
